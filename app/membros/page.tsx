...
// ✅ PADRÃO AGORA 6 DÍGITOS (consistente com sua UI nova)
...
useEffect(() => {
  ...
  const subfuncoesDisponiveis = useMemo(() => {
    const base = [...OPCOES_INSTRUMENTOS];
    const funcoesDoBanco = (membros || [])
      .map((m) => String(m?.funcao || '').trim())
      .filter(Boolean);
...
const salvarConfigsBanda = async () => {
  ...
  // 🔐 Só processa PIN se o usuário escolheu alterar
  ...
  if (alterarPin) {
    ...
    if (pinDigits.length !== 6) {
      return alert('O novo PIN deve ter exatamente 6 números.');
    }
  ...
  try {
    ...
    const payload: any = {
      nome: String(nomeBandaEdit || '').trim(),
      slug: novoSlug,
    };

    // ✅ Só inclui o PIN se for para alterar
    if (pinParaSalvar) {
      payload.pin_acesso = pinParaSalvar;
    }
          ...
          <div className="w-full max-w-md p-6 bg-yellow-500/10 border border-yellow-500/20 rounded-[2rem] text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-slate-400 mt-2">
                Você acessou esta página sem um convite de banda.
              </p>
            </div>
            <Link
              href="/"
              ...
              <div className="w-full max-w-md space-y-6 animate-in fade-in zoom-in duration-300">
                {!adminAutenticado ? (
                  <div className="bg-slate-900 border relative overflow-hidden border-white/5 p-10 rounded-[2.5rem] shadow-2xl text-center space-y-6">
                    ...
                    <p className="text-[12px] font-black text-white-500 uppercase tracking-[0.3em]">
...
<div className="bg-slate-950 p-4 rounded-xl border border-white/5 space-y-3">
  <div className="flex items-center justify-between">
    <div className="flex items-center gap-2">
      ...
      <span className="text-[12px] font-mono text-yellow-600 uppercase">
        PIN de acesso
      </span>
    </div>
  ...
  {!alterarPin ? (
    ...
    <input
      ...
      inputMode="numeric"
      onChange={(e) =>
        setNovoPinEdit(e.target.value.replace(/\D/g, '').slice(0, 6))
      }
    ...
    />
                    ...
                    <section className="bg-slate-900 border relative overflow-hidden border-white/5 p-8 rounded-[3rem] shadow-xl">
                      ...
                      <div className="bg-slate-950 p-4 rounded-xl flex items-center justify-between gap-3 border border-white/5">
                        ...
                        <button
                          ...
                          className="p-2 bg-slate-900 rounded-lg border border-white/10"
                          title="Copiar link"
                        >
                          {copiadoLink ? (
                      ...
                      <div className="space-y-3 max-h-[320px] overflow-y-auto no-scrollbar mt-4">
                        {membros.map((m) => (
                          ...
                          >
                            ...
                            <button
                              onClick={() => deletarMembro(m.id, m.nome)}
                    ...
                    <section className="bg-slate-900 border relative overflow-hidden border-white/5 p-8 rounded-[3rem] shadow-xl space-y-4">
                        ...
                        <div className="flex justify-between items-center bg-slate-950 p-4 rounded-xl border border-white/5">
                          ...
                          <span
                            className={`text-[10px] font-black uppercase px-2 py-1 rounded ${
                              isAtivo ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'
                            }`}
                          >
                            {statusAssinatura}
                          </span>
                      ...
                      {!isAtivo && (
                        <Link
                          href="/checkout"
                          className="block w-full bg-red-600 hover:bg-red-500 transition-colors py-4 rounded-xl font-black uppercase text-[10px] text-center shadow-lg shadow-red-900/20"
                        >
                          Regularizar Agora
                        </Link>
                      )}
                    ...
                    >
                      ...
                      Sair do Modo Admin
                                      ...
                                      <div className="absolute top-0 left-0 w-full relative h-1 bg-gradient-to-r from-transparent via-blue-500 to-transparent opacity-50" />
            ...
            {fase === 'cadastro' && (
              <form
                ...
                className="w-full max-w-md bg-slate-900/50 border overflow-hidden border-white/5 p-8 rounded-[2.5rem] space-y-6 relative shadow-2xl"
              >
                                  <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-blue-500 to-transparent opacity-50" />
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      name="nome"
                      placeholder="Ex: Lucas Teclas"
                      className="w-full p-4 rounded-xl bg-slate-950 text-white border border-white/5 outline-none focus:border-emerald-500/50 transition-all font-bold placeholder:text-slate-700"
                      required
                    />
                  </div>
                  ...
                  <div>
                    ...
                    {!cadUsarFuncaoCustom ? (
                      <>
                        <select
                          name="funcao"
                          className="w-full p-4 rounded-xl bg-slate-950 text-white border border-white/5 outline-none focus:border-emerald-500/50 font-bold appearance-none"
                          required
                          defaultValue=""
                        >
                      ...
                      <>
                        <input
                          value={cadFuncaoCustom}
                          onChange={(e) => setCadFuncaoCustom(e.target.value)}
                          placeholder="Digite sua função..."
                          className="w-full p-4 rounded-xl bg-slate-950 text-white border border-white/5 outline-none focus:border-blue-500/50 transition-all font-bold placeholder:text-slate-700"
                          required
                        />
                        <button
                          type="button"
                  ...
                  <div>
                    ...
                    <MultiSelectSubfuncoes
                      disponiveis={subfuncoesDisponiveis}
                      selecionadas={subfuncoesSelecionadas}
                      onChange={setSubfuncoesSelecionadas}
                    />
                    {subfuncoesSelecionadas.length > 0 && (
                      <button
                        type="button"
                        onClick={() => setSubfuncoesSelecionadas([])}
                        className="mt-3 text-[9px] font-black uppercase tracking-widest text-slate-500 hover:text-white transition-colors"
                      >
                        Limpar subfunções
                      </button>
              ...
              <div className="w-full max-w-md bg-slate-900 border overflow-hidden border-white/5 p-8 rounded-[2.5rem] shadow-2xl relative">
                {sucesso && (
                  <div className="absolute inset-0 bg-blue-600 flex flex-col items-center justify-center z-10 animate-in fade-in duration-300 rounded-[2.5rem]">
                    ...
                    <span className="text-white font-black uppercase text-xs tracking-[0.3em] mt-4">
                      Perfil Ativado!
                    </span>
                  </div>
                )}
                    ...
                    <div className="p-5 bg-blue-600/5 border border-blue-500/10 rounded-2xl space-y-4">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          {!editandoAtivo ? (
                            <button
                              onClick={abrirEdicaoAtivo}
                              className="p-3 bg-slate-950 rounded-xl border border-white/5 text-slate-500 hover:text-blue-400 hover:border-blue-500/30 transition-all"
                            >
                              <Edit3 size={18} />
                            </button>
                          ) : (
                          ...
                          <button
                            ...
                            className="p-3 bg-slate-950 rounded-xl border border-white/5 text-slate-600 hover:text-red-500 hover:border-red-500/30 transition-all"
                          >
                            <LogOut size={18} />
                          </button>
                        </div>
                        ...
                        <div className="space-y-4">
                          ...
                          <div>
                            ...
                            {!USE_MULTISELECT_SUBFUNCOES ? (
                              <MultiSelectSubfuncoes
                                disponiveis={subfuncoesDisponiveis}
                                selecionadas={editSubfuncoes}
                                onChange={setEditSubfuncoes}
                              />
                            ) : (
                              <select
                                multiple
                          ...
                          <div className="pt-1">
                            <p className="text-[9px] font-black uppercase tracking-widest text-slate-600">
                              {salvandoEdicao ? 'Salvando...' : 'Edite e clique em salvar.'}
                            </p>
                          </div>
                        </div>
                      )}
